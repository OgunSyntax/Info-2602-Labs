...
# Models are classes that become tables.
# objects are created from these models, which reflect a table row record.
class User(db.Model):
...
#  Constructor of the class used to make class instances
  def __init__(self, username, email, password):
    self.username = username
    self.email = email
    self.set_password(password)

# This is a method which handles user accounts. It uses hashes for the user passwords+
  def set_password(self, password):
    """Create hashed password."""
    self.password = generate_password_hash(password, method='scrypt')


# This method returns a string representation. This prints the objects to the console
  def __repr__(self):
